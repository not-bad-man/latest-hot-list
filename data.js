...
var data = fs.readFileSync('./index.txt').toString();
...
for (var i = 0; i < len; i++) {
   var tempData = newData[i].split('\t');
   var random = Math.ceil( Math.random() * 1000 );
   var oldCount = Math.random() > 0.5 ? parseInt(tempData[2]) + random : parseInt(tempData[2]) - random;
   dataArray.push({
      index: tempData[0], 
      tip: tempData[1], 
      newCount: parseInt(tempData[2]),
      oldCount: oldCount
   })
}
...
var data = [ { index: '1', tip: '马云的福字', newCount: 489145, oldCount: 489028 },
{ index: '2',
  tip: '清华硕士荷兰丧生',
  newCount: 447412,
  oldCount: 447506 },
